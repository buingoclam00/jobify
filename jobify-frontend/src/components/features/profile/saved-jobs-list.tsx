...
const SavedJobsList = ({ userId }: SavedJobsListProps) => {
  ...
  const {
    ...
    () => savedJobsApi.getUserSavedJobs(userId),
    [],
    { immediate: true }
  );
  ...
  const handleUnsaveJob = async (savedJobId: string) => {
    try {
      // TODO: Implement unsave API call
      // await savedJobsApi.unsaveJob(savedJobId);
        ...
        <Card>
          <CardContent className="text-center py-12">
            ...
            <h3 className="text-lg font-semibold text-gray-900 mb-2">
              {savedJobs?.data?.length === 0 ? 'Chưa có việc làm đã lưu' : 'Không tìm thấy kết quả'}
            </h3>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="mt-4 pt-4 border-t border-gray-200">
                          <div className="flex items-center gap-2 text-xs text-gray-500">
                            ...
                            <span>Đã lưu {formatRelativeTime(savedJob.createdAt)}</span>
                          </div>
                        </div>
                      </div>
                    </div>
