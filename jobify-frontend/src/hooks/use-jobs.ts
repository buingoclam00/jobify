...
export function useJobs(options: UseJobsOptions = {}) {
  ...
  const apiCall = useCallback((page: number, pageLimit: number) => {
    console.log('API call triggered', { page, pageLimit, filters: memoizedFilters });
  ...
  const updateURL = useCallback((newFilters: JobSearchFilters) => {
  ...
  const search = useCallback((search: string, location?: string) => {
    updateFilters({
      search,
  ...
  // Change sort
  const changeSort = useCallback((sortBy: JobSearchFilters['sortBy'], sortOrder: JobSearchFilters['sortOrder']) => {
