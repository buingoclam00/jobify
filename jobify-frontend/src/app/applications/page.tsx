...
const ApplicationsPage = () => {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await applicationsApi.getApplicationsByCompanyDetailed(user._id);
      if (response?.data) {
  ...
  const filteredApplications = applications
    .filter(app => {
      ...
      const matchesCompany = companyFilter === 'all' || app.jobPostId.companyId._id === companyFilter;
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 mb-3">
                                  ...
                                  <div className="flex items-center gap-1">
                                    ...
                                    <span>{application.jobPostId.location}</span>
                                  </div>
                                  {(application.jobPostId.salaryMin || application.jobPostId.salaryMax) && (
                                    <div className="flex items-center gap-1">
                                      <DollarSign className="w-4 h-4" />
                                      <span>{formatSalaryRange(application.jobPostId.salaryMin, application.jobPostId.salaryMax)}</span>
                                    </div>
                                  )}
                                  <div className="flex items-center gap-1">
                                ...
                                {/* Resume */}
                                {(application.resumeUrl || application.userId.resumeUrl) && (
                                  <div className="flex items-center gap-2 mb-3">
                                    <FileText className="w-4 h-4 text-blue-600" />
                                    <a
                                      href={application.resumeUrl || application.userId.resumeUrl}
                                      target="_blank"
                              ...
                              <div className="flex flex-col items-end gap-3">
                                <div className="flex gap-2">
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => window.open(`/jobs/${application.jobPostId._id}`, '_blank')}
                                  >
